...
def compute_posterior(prior, likelihood, y):
    ...
    for i in range(M):
    	for j in range(len(y)):
    		num[i] = num[i] + log_likelihood[y[j], i]
...
def compute_movie_rating_likelihood(M):
    ...
    for i in range(M):
    	for j in range(M):
    		if (i == j):
    			likelihood[j, i] = 2*alpha
    		else:
    			likelihood[j, i] = alpha / (np.absolute(j - i))
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    if (num_observations == -1):
    	for i in range(num_movies):
    		ratings = movie_data_helper.get_ratings(movie_id_list[i])[:None]
    		posteriors[i] = compute_posterior(prior, likelihood, ratings)
    		MAP_ratings[i] = np.argmax(posteriors[i])
    else:
    	for i in range(num_movies):
    		ratings = movie_data_helper.get_ratings(movie_id_list[i])[0:num_observations]
    		posteriors[i] = compute_posterior(prior, likelihood, ratings)
    		MAP_ratings[i] = np.argmax(posteriors[i])
...
# This function is specifically for use in plotting the posterior entropies
# Without it, pulling that data over and over takes forever
def plot_entropies():
    ...
    print("Computing entropies for plotting (this should take about a minute) ... ")
    for i in range(num_movies):
...
def main():
    ...
    # Below is my original, unoptimized, version which took forever but I am
    # including it here in case for some reason this is what you're looking for

    # Test and print the compute_true_movie_rating_posterior_entropies method
    # print("Preparing the posterior entropies now ... ")
    # posterior_array = []
    # for n in range(1, 201):
        # posterior_array.append(np.mean(compute_true_movie_rating_posterior_entropies(n)))
    # print("Finished computing. First ten are: ")
    # print(posterior_array[0:10])
